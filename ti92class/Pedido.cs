...
namespace ti92class
{
    public class Pedido
    {
        ...
        public static List<Pedido> Listar()
        {
            ...
            var cmd = Banco.Abrir();
            cmd.CommandText = "select * from pedidos where arquivado_em is null order by id desc"; //* indica que vai aparecer na ordem do banco; 
        ...
        public List<Pedido> Arquivados()
        {
            List<Pedido> list = new List<Pedido>();
            var cmd = Banco.Abrir();
            cmd.CommandText = "select * from pedidos where arquivado_em is not null order by arquivado_em desc;"; //* indica que vai aparecer na ordem do banco; 
            var dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                list.Add(new Pedido(
